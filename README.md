...
## 模型簡介
### DenseNet121
又稱密集卷積網絡(Dense Connection)，基本思路與ResNet一致，以前饋方式將每一層連接到其他每一層。而具有L層的傳統卷積網絡有L個連接——每層與其後續層之間有一個連接——而我們的網絡有L(L+1)/2 個直接連接。對於每一層，所有先前層的特徵圖用作輸入，其自身的特徵圖用作所有後續層的輸入，使DenseNet有在參數與計算成本更少的優點，有著比ResNe更優的性能。
